...
var import_obsidian6 = require("obsidian"), import_fs2 = __toESM(require("fs")), import_path2 = __toESM(require("path"));
...
var import_jsx_runtime2 = __toESM(require_jsx_runtime()), Search = () => {
  ...
  ), [query, setQuery] = (0, import_react2.useState)(""), [selectedResultIndex, setSelectedResultIndex] = (0, import_react2.useState)(0), resultsDiv = (0, import_react2.useRef)(null);
  (0, import_react2.useEffect)(() => {
    let unbind = settingsObservable == null ? void 0 : settingsObservable.onChange((newSettings) => {
      setSettings(newSettings);
    });
    return () => {
      typeof unbind == "function" && unbind();
    };
  }, [settingsObservable]), (0, import_react2.useEffect)(() => {
    resultsDiv.current && resultsDiv.current.scrollTo({
      top: (selectedResultIndex - 2) * 40,
      behavior: "smooth"
    });
  }, [selectedResultIndex]);
  let searchResult = (0, import_react2.useMemo)(() => {
    if (!query || query.trim().length === 0)
      return [];
    let cleanQuery = query.toLowerCase(), localMusicItems = ((settings == null ? void 0 : settings.myMusicIndex) || []).map((song) => ({
      type: "LOCAL",
      id: song.fileName || "",
    ...
    })), defaultSoundscapeItems = Object.values(Soundscapes_default || {}).map((sc) => ({
      type: "STANDARD",
      id: sc.id || "",
      fileName: sc.id || "",
    ...
    return ((settings == null ? void 0 : settings.customSoundscapes) || []).forEach((customGroup) => {
      ...
      (item) => String(item.title || "").toLowerCase().includes(cleanQuery) || String(item.artist || "").toLowerCase().includes(cleanQuery) || String(item.album || "").toLowerCase().includes(cleanQuery) || String(item.id || "").toLowerCase().includes(cleanQuery)
    ).slice(0, 20);
  }, [settings == null ? void 0 : settings.myMusicIndex, settings == null ? void 0 : settings.customSoundscapes, query]), playSelectedItem = (item) => {
    var _a, _b, _c, _d;
    if (plugin) {
      if (item.type === "LOCAL") {
        let parentCollection = (_a = settings == null ? void 0 : settings.musicCollections) == null ? void 0 : _a.find(
          (collection) => {
            var _a2;
            return (_a2 = collection.tracks || collection.files || []) == null ? void 0 : _a2.some((t) => t.fileName === item.fileName);
          }
        );
        parentCollection && (plugin.settings.soundscape = `MUSIC_COLLECTION_${parentCollection.id}`, plugin.saveSettings()), plugin.changeMyMusicTrack(item.fileName);
      } else if (item.type === "STANDARD")
        plugin.changeSoundscape(item.id);
      else if (item.type === "CUSTOM") {
        plugin.settings.soundscape = `CUSTOM_${item.customGroupId}`, plugin.saveSettings();
        let customGroup = (_b = settings == null ? void 0 : settings.customSoundscapes) == null ? void 0 : _b.find((g) => g.id === item.customGroupId), trackIdx = (_d = (_c = customGroup == null ? void 0 : customGroup.tracks) == null ? void 0 : _c.findIndex((t) => t.id === item.id)) != null ? _d : 0;
        plugin.currentTrackIndex = trackIdx >= 0 ? trackIdx : 0, plugin.onSoundscapeChange();
      }
      setQuery("");
    }
  };
  return /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "soundscapesmymusic-right-search", children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(
      "input",
      {
        ...
        value: query,
        onChange: (e) => {
          setQuery(e.target.value), setSelectedResultIndex(0);
        },
        onKeyDown: (e) => {
          if (searchResult.length > 0)
            switch (e.key) {
              case "ArrowDown":
                e.preventDefault(), setSelectedResultIndex(
                  (prev) => prev === searchResult.length - 1 ? 0 : prev + 1
                );
                break;
              case "ArrowUp":
                e.preventDefault(), setSelectedResultIndex(
                  (prev) => prev === 0 ? searchResult.length - 1 : prev - 1
                );
                break;
              case "Enter": {
                e.preventDefault();
                let currentActiveItem = searchResult[selectedResultIndex];
                currentActiveItem && playSelectedItem(currentActiveItem);
                break;
              }
              case "Escape":
                setQuery("");
    ...
    query.trim().length > 0 && /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "soundscapesmymusic-right-search-results", ref: resultsDiv, children: searchResult.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "soundscapesmymusic-right-search-results-message", children: "No results found" }) : searchResult.map((item, index) => /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "soundscapesmymusic-right-search-results-result-line1", children: item.title }),
          /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "soundscapesmymusic-right-search-results-result-line2", children: item.artist || item.album })
        ]
      },
      `${item.type}-${item.id}`
    )) })
...
var import_jsx_runtime3 = __toESM(require_jsx_runtime()), Header = () => {
  ...
  ), [settings, setSettings] = (0, import_react3.useState)(((_a = plugin == null ? void 0 : plugin.settingsObservable) == null ? void 0 : _a.getValue()) || (plugin == null ? void 0 : plugin.settings)), titleElementRef = (0, import_react3.useRef)(null), placeholderRef = (0, import_react3.useRef)(null), shouldScrollTitle = titleElementRef != null && titleElementRef.current ? ((_b = titleElementRef == null ? void 0 : titleElementRef.current) == null ? void 0 : _b.scrollWidth) > ((_c = titleElementRef == null ? void 0 : titleElementRef.current) == null ? void 0 : _c.clientWidth) : !1;
  (0, import_react3.useEffect)(() => {
    var _a2;
    let unsubscribeState = localPlayerStateObservable == null ? void 0 : localPlayerStateObservable.onChange(
      (newState) => {
        setLocalPlayerState(newState);
      }
    ), unsubscribeSettings = (_a2 = plugin == null ? void 0 : plugin.settingsObservable) == null ? void 0 : _a2.onChange(
      (newSettings) => {
        setSettings(newSettings);
      }
    );
    return () => {
      unsubscribeState == null || unsubscribeState(), unsubscribeSettings == null || unsubscribeSettings();
    };
  }, [localPlayerStateObservable, plugin]);
  let activeSoundscape = (settings == null ? void 0 : settings.soundscape) || "lofi", isLocalActive = activeSoundscape.startsWith("MUSIC_COLLECTION_"), isCustomYoutubeActive = activeSoundscape.startsWith("CUSTOM_"), activeTitle = "", activeArtist = "", durationSeconds = 0, elapsedSeconds = 0;
  if (isLocalActive)
    activeTitle = ((_d = localPlayerState == null ? void 0 : localPlayerState.currentTrack) == null ? void 0 : _d.title) || ((_e = localPlayerState == null ? void 0 : localPlayerState.currentTrack) == null ? void 0 : _e.fileName) || "", activeArtist = ((_f = localPlayerState == null ? void 0 : localPlayerState.currentTrack) == null ? void 0 : _f.artist) || "Local Audio", durationSeconds = ((_g = localPlayerState == null ? void 0 : localPlayerState.currentTrack) == null ? void 0 : _g.duration) || 0, elapsedSeconds = (localPlayerState == null ? void 0 : localPlayerState.currentTime) || 0;
  else if (isCustomYoutubeActive) {
    let playlistId = activeSoundscape.replace("CUSTOM_", ""), currentPlaylist = (_h = settings == null ? void 0 : settings.customSoundscapes) == null ? void 0 : _h.find((p) => p.id === playlistId), trackIndex = (_i = plugin == null ? void 0 : plugin.currentTrackIndex) != null ? _i : 0, activeTrack = (_j = currentPlaylist == null ? void 0 : currentPlaylist.tracks) == null ? void 0 : _j[trackIndex];
    activeTitle = (activeTrack == null ? void 0 : activeTrack.name) || "Loading YouTube Track...", activeArtist = (activeTrack == null ? void 0 : activeTrack.author) || (activeTrack == null ? void 0 : activeTrack.channelName) || "YouTube Playlist", durationSeconds = ((_k = localPlayerState == null ? void 0 : localPlayerState.currentTrack) == null ? void 0 : _k.duration) || (activeTrack == null ? void 0 : activeTrack.duration) || 0, elapsedSeconds = (localPlayerState == null ? void 0 : localPlayerState.currentTime) || 0;
  } else {
    let ambientItem = Soundscapes_default[activeSoundscape];
    ambientItem && (activeTitle = ambientItem.name, activeArtist = "Ambient Radio");
  }
  (0, import_react3.useEffect)(() => {
    let ytPlayerElement = document.getElementById("player"), placeholder = placeholderRef.current;
    if (!ytPlayerElement)
      return;
    if (!placeholder || isLocalActive) {
      ytPlayerElement.style.removeProperty("position"), ytPlayerElement.style.removeProperty("display"), ytPlayerElement.style.removeProperty("left"), ytPlayerElement.style.removeProperty("top"), ytPlayerElement.style.removeProperty("width"), ytPlayerElement.style.removeProperty("height"), ytPlayerElement.style.removeProperty("border-radius"), ytPlayerElement.style.removeProperty("z-index");
      return;
    }
    let updatePosition = () => {
      if (!placeholder || !ytPlayerElement)
        return;
      let rect = placeholder.getBoundingClientRect();
      rect.width === 0 || rect.height === 0 || rect.top === 0 || (ytPlayerElement.style.setProperty("position", "fixed", "important"), ytPlayerElement.style.setProperty("display", "block", "important"), ytPlayerElement.style.setProperty("left", `${rect.left}px`, "important"), ytPlayerElement.style.setProperty("top", `${rect.top}px`, "important"), ytPlayerElement.style.setProperty("width", `${rect.width}px`, "important"), ytPlayerElement.style.setProperty("height", `${rect.height}px`, "important"), ytPlayerElement.style.setProperty("border-radius", "6px", "important"), ytPlayerElement.style.setProperty("z-index", "1000", "important"));
    }, resizeObserver = new ResizeObserver(() => updatePosition());
    resizeObserver.observe(placeholder), window.addEventListener("resize", updatePosition);
    let scrollContainers = document.querySelectorAll(".view-content, .workspace-leaf-content");
    return scrollContainers.forEach((container) => container.addEventListener("scroll", updatePosition)), updatePosition(), () => {
      resizeObserver.disconnect(), window.removeEventListener("resize", updatePosition), scrollContainers.forEach((container) => container.removeEventListener("scroll", updatePosition)), ytPlayerElement.style.removeProperty("position"), ytPlayerElement.style.removeProperty("display"), ytPlayerElement.style.removeProperty("left"), ytPlayerElement.style.removeProperty("top"), ytPlayerElement.style.removeProperty("width"), ytPlayerElement.style.removeProperty("height"), ytPlayerElement.style.removeProperty("border-radius"), ytPlayerElement.style.removeProperty("z-index");
    };
  }, [isLocalActive, settings == null ? void 0 : settings.soundscape, localPlayerState == null ? void 0 : localPlayerState.currentTrack]);
  let hasTrackInfo = !!activeTitle, isLiveStream = !isLocalActive && (!durationSeconds || durationSeconds === 0);
  return /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { className: "soundscapesmymusic-header", children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { className: "soundscapesmymusic-volume", children: /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(
      "input",
      {
        ...
        min: "0",
        max: "100",
        value: (_l = settings == null ? void 0 : settings.volume) != null ? _l : 50,
        onChange: (e) => plugin == null ? void 0 : plugin.onVolumeChange(e)
      }
    ) }),
    /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { className: "soundscapesmymusic-middle", children: hasTrackInfo && /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)(import_jsx_runtime3.Fragment, { children: [
      !isLocalActive && /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(
        "div",
        {
          ref: placeholderRef,
      ...
      /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { className: "soundscapesmymusic-middle-line1", children: [
        /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { className: "soundscapesmymusic-middle-line1-left", children: /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(
          "button",
          {
            className: `soundscapesmymusic-middle-line1-button ${(settings == null ? void 0 : settings.playMode) === "shuffle" && "soundscapesmymusic-middle-line1-button--active"}`,
            onClick: () => {
        ...
        /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(
          "div",
          {
            ...
            ref: titleElementRef,
            children: /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("span", { className: "soundscapesmymusic-middle-line1-title-text", children: activeTitle })
          }
        ),
      ...
      /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { className: "soundscapesmymusic-middle-line2", children: [
        /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { className: "soundscapesmymusic-middle-line2-left", children: isLiveStream ? "00:00" : secondsToMinutesAndSeconds_default(elapsedSeconds) }),
        /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { className: "soundscapesmymusic-middle-line2-artist", children: activeArtist }),
        /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { className: "soundscapesmymusic-middle-line2-right", children: isLiveStream ? "Stream" : `-${secondsToMinutesAndSeconds_default(Math.max(0, durationSeconds - elapsedSeconds))}` })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(
        "input",
        {
          ...
          onChange: (e) => {
            var _a2, _b2;
            let targetSeconds = parseFloat(e.target.value);
            isLocalActive ? plugin == null || plugin.seek(targetSeconds) : isCustomYoutubeActive && (plugin != null && plugin.player) && ((_b2 = (_a2 = plugin.player).seekTo) == null || _b2.call(_a2, targetSeconds, !0), plugin.updateLocalPlayerState({ currentTime: targetSeconds }));
          },
          className: "soundscapesmymusic-middle-seekbar"
        }
      )
    ] }) }),
    /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { className: "soundscapesmymusic-right", children: /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { className: "soundscapes-search-container", children: /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(Search_default, {}) }) })
  ] });
}, Header_default = Header;

// React/Components/App/App.tsx
var import_jsx_runtime4 = __toESM(require_jsx_runtime()), App = () => {
  let { settingsObservable, localPlayerStateObservable, plugin } = useObsidianPluginContext(), [settings, setSettings] = (0, import_react4.useState)(settingsObservable == null ? void 0 : settingsObservable.getValue()), [localPlayerState, setLocalPlayerState] = (0, import_react4.useState)(localPlayerStateObservable == null ? void 0 : localPlayerStateObservable.getValue()), layoutStyle = settings && settings.layoutStyle || "split", [activeView, setActiveView] = (0, import_react4.useState)({ type: "home", id: null, label: "Library" }), [ambientExpanded, setAmbientExpanded] = (0, import_react4.useState)(!1), [youtubeExpanded, setYoutubeExpanded] = (0, import_react4.useState)(!0), [localExpanded, setLocalExpanded] = (0, import_react4.useState)(!0), handleSoundscapeChange = (0, import_react4.useCallback)((soundscapeId) => {
    plugin && typeof plugin.changeSoundscape == "function" && plugin.changeSoundscape(soundscapeId);
  }, [plugin]), syncViewFromSoundscapeId = (0, import_react4.useCallback)((soundscapeId) => {
    var _a, _b;
    ...
    if (soundscapeId.startsWith("CUSTOM_")) {
      let targetId = soundscapeId.replace("CUSTOM_", ""), targetPlaylist = (_a = settings.customSoundscapes) == null ? void 0 : _a.find((p) => p.id === targetId);
      setActiveView({
        type: "youtube",
        id: targetId,
      ...
      let targetId = soundscapeId.replace("MUSIC_COLLECTION_", ""), targetCollection = (_b = settings.musicCollections) == null ? void 0 : _b.find((c) => c.id === targetId);
      setActiveView({
        type: "local",
        id: targetId,
      ...
      let ambient = Soundscapes_default[soundscapeId];
      setActiveView(ambient ? { type: "ambient", id: soundscapeId, label: ambient.name } : { type: "home", id: null, label: "Library" });
    }
  }, [settings]);
  (0, import_react4.useEffect)(() => {
    let unsubscribe = settingsObservable == null ? void 0 : settingsObservable.onChange((newSettings) => {
      setSettings(newSettings);
    });
    return () => unsubscribe == null ? void 0 : unsubscribe();
  }, [settingsObservable]), (0, import_react4.useEffect)(() => {
    let unsubscribe = localPlayerStateObservable == null ? void 0 : localPlayerStateObservable.onChange((newState) => {
      setLocalPlayerState(newState);
    });
    return () => unsubscribe == null ? void 0 : unsubscribe();
  }, [localPlayerStateObservable]), (0, import_react4.useEffect)(() => {
    settings != null && settings.soundscape && (syncViewFromSoundscapeId(settings.soundscape), !settings.soundscape.startsWith("CUSTOM_") && !settings.soundscape.startsWith("MUSIC_COLLECTION_") && setAmbientExpanded(!0));
  }, [settings == null ? void 0 : settings.soundscape, syncViewFromSoundscapeId]);
  let navigationItems = (0, import_react4.useMemo)(() => {
    let items = [];
    return settings && (settings.customSoundscapes && settings.customSoundscapes.length > 0 && settings.customSoundscapes.forEach((list) => {
      items.push({ type: "youtube", id: list.id, name: list.name, icon: "youtube" });
    }), settings.musicCollections && settings.musicCollections.length > 0 ? settings.musicCollections.forEach((collection) => {
      items.push({ type: "local", id: collection.id, name: collection.name, icon: "folder" });
    }) : settings.myMusicIndex && settings.myMusicIndex.length > 0 && items.push({ type: "local", id: "legacy_root", name: "Local Music Library", icon: "music" })), items;
  }, [settings == null ? void 0 : settings.customSoundscapes, settings == null ? void 0 : settings.musicCollections, settings == null ? void 0 : settings.myMusicIndex]), currentTracks = (0, import_react4.useMemo)(() => {
    var _a;
    if (!settings)
      return [];
    if (activeView.type === "ambient")
      if (activeView.id) {
        let stream2 = Soundscapes_default[activeView.id];
        return stream2 ? [{
          id: stream2.id,
          title: stream2.name,
          artist: "Ambient Stream",
          ...
          duration: 0,
          source: "ambient",
          nativeTrackRef: stream2
        }] : [];
      } else
        return Object.values(Soundscapes_default).map((stream2) => ({
          id: stream2.id,
          title: stream2.name,
          artist: "Ambient Stream",
          album: "Soundscapes",
    ...
    if (activeView.type === "youtube" && activeView.id) {
      let targetPlaylist = (_a = settings.customSoundscapes) == null ? void 0 : _a.find((p) => p.id === activeView.id);
      return !targetPlaylist || !targetPlaylist.tracks ? [] : targetPlaylist.tracks.map((track, idx) => {
        let ytTrack = track;
        return {
          id: `${track.id}_${idx}`,
          title: track.name,
    ...
    return activeView.type === "local" ? (settings.myMusicIndex || []).map((song) => ({
      id: song.fullPath,
  ...
  }, [activeView, settings == null ? void 0 : settings.customSoundscapes, settings == null ? void 0 : settings.myMusicIndex]), handleTrackPlay = (0, import_react4.useCallback)((track) => {
    plugin && (track.source === "local" ? plugin.changeMyMusicTrack(track.nativeTrackRef) : (track.source === "youtube" || track.source === "ambient") && plugin.changeMyMusicTrack(track.id));
  }, [plugin]), renderNavigationList = () => {
    let youtubeItems = (settings == null ? void 0 : settings.customSoundscapes) || [], localCollections = (settings == null ? void 0 : settings.musicCollections) || [];
    return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "soundscapes-nav-panel", children: [
      /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("div", { className: "soundscapes-nav-header", children: "Library Sources" }),
      /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "soundscapes-nav-group", children: [
        /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(
          "div",
          {
            className: "soundscapes-nav-group-header",
            onClick: () => {
              setAmbientExpanded(!ambientExpanded), setActiveView({ type: "ambient", id: null, label: "Ambient Streams" });
            },
            style: { display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", padding: "6px 10px", color: "var(--text-muted)" },
            children: [
              /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { style: { fontSize: "0.75rem", width: "12px", textAlign: "center" }, children: ambientExpanded ? "\u25BC" : "\u25B6" }),
              /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Icon_default, { name: "radio" }),
              /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { className: "soundscapes-nav-text", style: { fontWeight: 500 }, children: "Ambient Streams" })
            ]
          }
        ),
        ambientExpanded && /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("div", { className: "soundscapes-nav-group-items", style: { marginLeft: "14px", display: "flex", flexDirection: "column", gap: "2px" }, children: Object.values(Soundscapes_default).map((stream2) => {
          let isSelected = activeView.type === "ambient" && activeView.id === stream2.id;
          return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(
            "div",
            {
              className: `soundscapes-nav-item ${isSelected ? "is-active" : ""}`,
              onClick: (e) => {
                e.stopPropagation(), handleSoundscapeChange(stream2.id);
              },
              children: [
                /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { className: "soundscapes-nav-icon", children: "\u{1F3B5}" }),
                /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { className: "soundscapes-nav-text", children: stream2.name })
              ]
            },
            stream2.id
          );
      ...
      /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "soundscapes-nav-group", style: { marginTop: "8px" }, children: [
        /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(
          "div",
          {
            className: "soundscapes-nav-group-header",
            ...
            style: { display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", padding: "6px 10px", color: "var(--text-muted)" },
            children: [
              /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { style: { fontSize: "0.75rem", width: "12px", textAlign: "center" }, children: youtubeExpanded ? "\u25BC" : "\u25B6" }),
              /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Icon_default, { name: "youtube" }),
              /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { className: "soundscapes-nav-text", style: { fontWeight: 500 }, children: "YouTube Playlists" })
            ]
          }
        ),
        youtubeExpanded && /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "soundscapes-nav-group-items", style: { marginLeft: "14px", display: "flex", flexDirection: "column", gap: "2px" }, children: [
          youtubeItems.map((list) => {
            let isSelected = activeView.type === "youtube" && activeView.id === list.id;
            return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(
              "div",
              {
                className: `soundscapes-nav-item ${isSelected ? "is-active" : ""}`,
                ...
                children: [
                  /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Icon_default, { name: "youtube" }),
                  /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { className: "soundscapes-nav-text", children: list.name })
                ]
              },
              list.id
            );
          }),
          youtubeItems.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("div", { style: { padding: "4px 24px", fontSize: "0.8em", color: "var(--text-muted)" }, children: "No custom streams" })
        ] })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "soundscapes-nav-group", style: { marginTop: "8px" }, children: [
        /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(
          "div",
          {
            className: "soundscapes-nav-group-header",
            onClick: () => {
              setLocalExpanded(!localExpanded), setActiveView({ type: "local", id: null, label: "Local Music" });
            },
            style: { display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", padding: "6px 10px", color: "var(--text-muted)" },
            children: [
              /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { style: { fontSize: "0.75rem", width: "12px", textAlign: "center" }, children: localExpanded ? "\u25BC" : "\u25B6" }),
              /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Icon_default, { name: "folder" }),
              /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { className: "soundscapes-nav-text", style: { fontWeight: 500 }, children: "Collections" })
            ]
          }
        ),
        localExpanded && /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "soundscapes-nav-group-items", style: { marginLeft: "14px", display: "flex", flexDirection: "column", gap: "2px" }, children: [
          localCollections.map((collection) => {
            let isSelected = activeView.type === "local" && activeView.id === collection.id;
            return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(
              "div",
              {
                className: `soundscapes-nav-item ${isSelected ? "is-active" : ""}`,
                ...
                children: [
                  /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Icon_default, { name: "folder" }),
                  /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { className: "soundscapes-nav-text", children: collection.name })
                ]
              },
              collection.id
            );
          }),
          localCollections.length === 0 && (settings.myMusicIndex || []).length > 0 && /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(
            "div",
            {
              ...
              children: [
                /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Icon_default, { name: "music" }),
                /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { className: "soundscapes-nav-text", children: "All Local Music" })
              ]
            }
          )
        ] })
  ...
  }, renderTrackTable = () => /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "soundscapesmymusic-musiclist", children: [
    activeView.type !== "home" && layoutStyle === "drilldown" && /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(
      "button",
      {
    ...
    /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("table", { className: "soundscapesmymusic-musiclist-table", children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("tbody", { children: [
        currentTracks.map((track) => {
          var _a, _b;
          let isCurrentLocal = ((_a = localPlayerState == null ? void 0 : localPlayerState.currentTrack) == null ? void 0 : _a.fileName) === ((_b = track.nativeTrackRef) == null ? void 0 : _b.fileName) && track.source === "local", isCurrentStreaming = (settings == null ? void 0 : settings.soundscape) === activeView.id || track.source === "youtube" && (settings == null ? void 0 : settings.soundscape) === `CUSTOM_${activeView.id}`, isThisRowPlaying = isCurrentLocal || isCurrentStreaming;
          return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("tr", { onClick: () => handleTrackPlay(track), style: { cursor: "pointer" }, children: [
            /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("td", { children: [
              isThisRowPlaying && (localPlayerState == null ? void 0 : localPlayerState.playerState) === 1 /* PLAYING */ && /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Icon_default, { name: "volume-2" }),
              isThisRowPlaying && (localPlayerState == null ? void 0 : localPlayerState.playerState) === 2 /* PAUSED */ && /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Icon_default, { name: "volume" })
            ] }),
            /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("td", { children: track.title }),
            /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("td", { children: track.artist }),
            /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("td", { children: track.album }),
            /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("td", { children: track.duration > 0 ? secondsToMinutesAndSeconds_default(track.duration) : "Stream" })
          ] }, track.id);
        }),
        currentTracks.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("tr", { children: /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("td", { colSpan: 5, style: { textAlign: "center", padding: "20px", color: "var(--text-muted)" }, children: "No tracks found. Select a source or check your configurations." }) })
      ] })
    ] })
  ] });
  return settings ? /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "soundscapes-holistic-container", children: [
    /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Header_default, {}),
    layoutStyle === "split" ? /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "soundscapes-split-workspace", style: { display: "flex", height: "100%" }, children: [
      /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("aside", { className: "soundscapes-sidebar-aside", style: { width: "220px", borderRight: "1px solid var(--background-modifier-border)", overflowY: "auto" }, children: renderNavigationList() }),
      /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("main", { className: "soundscapes-main-content", style: { flex: 1, position: "relative" }, children: renderTrackTable() })
    ] }) : /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("div", { className: "soundscapes-drilldown-workspace", style: { height: "100%", position: "relative" }, children: activeView.type === "home" ? renderNavigationList() : renderTrackTable() })
  ] }) : null;
}, App_default = App;
...
var import_fs = __toESM(require("fs")), import_path = __toESM(require("path")), MUSIC_FILE_EXTENSIONS = ["mp3", "m4a", "opus", "ogg", "wav", "flac", "webm"], MIME_TYPES = {
  mp3: "audio/mp3",
  m4a: "audio/mp4",
  opus: "audio/ogg",
  // Opus is technically a subset of Ogg containers in HTML5
  ogg: "audio/ogg",
  wav: "audio/wav",
  flac: "audio/flac",
  webm: "audio/webm"
}, getMimeType = (extension) => MIME_TYPES[extension] || "audio/mpeg", getAllMusicFiles = (dirPath, fileArray = void 0) => {
  ...
  return fileArray = fileArray || [], files.forEach((file) => {
    ...
    import_fs.default.statSync(filePath).isDirectory() ? fileArray = getAllMusicFiles(filePath, fileArray) : MUSIC_FILE_EXTENSIONS.includes(import_path.default.extname(filePath).slice(1).toLowerCase()) && (fileArray == null || fileArray.push(filePath));
  }), fileArray;
...
var import_electron = __toESM(require("electron")), DEFAULT_SETTINGS = {
  ...
  display() {
    ...
    containerEl.empty(), new import_obsidian5.Setting(containerEl).setName("Soundscape").setDesc("Which soundscape would you like to listen to?").addDropdown((component) => {
      ...
      }), component.setValue(this.plugin.settings.soundscape), component.onChange((value) => {
    ...
    }), containerEl.createEl("h1", { text: "Music Collections" }), containerEl.createEl("p", {
      ...
      (collection, index) => {
        let isActive = this.plugin.settings.soundscape === `MUSIC_COLLECTION_${collection.id}`, settingEl = new import_obsidian5.Setting(containerEl).setName(collection.name).setDesc(
          `${collection.musicIndex.length} songs \u2014 ${collection.folderPath || "No path set"}`
        ).addText((component) => {
          component.setValue(collection.name), component.onChange((value) => {
            collection.name = value, this.plugin.saveSettings(), this.plugin.populateChangeSoundscapeButton(), settingEl.setName(value);
          });
          ...
          component.setIcon("folder-open"), component.setTooltip("Select folder"), component.onClick(() => {
            import_electron.default.remote.dialog.showOpenDialog({
              ...
              result.canceled || (collection.folderPath = result.filePaths[0], collection.musicIndex = [], isActive && (this.plugin.settings.myMusicIndex = [], this.plugin.settings.myMusicFolderPath = collection.folderPath), this.plugin.saveSettings(), this.display(), isActive && (this.plugin.indexMusicLibrary(), this.plugin.onSoundscapeChange()));
            });
          });
        }).addExtraButton((component) => {
          component.setIcon("folder-sync"), component.setTooltip("Re-index now"), component.onClick(() => {
            if (isActive)
              ...
              let prevSoundscape = this.plugin.settings.soundscape;
              this.plugin.changeSoundscape(`MUSIC_COLLECTION_${collection.id}`), this.plugin.indexMusicLibrary(), this.plugin.populateChangeSoundscapeButton(), this.display();
            }
          });
        }).addButton((component) => {
          component.setButtonText("Remove"), component.setClass("mod-warning"), component.onClick(() => {
            new ConfirmModal_default(
              this.plugin.app,
              () => {
                isActive && (this.plugin.settings.soundscape = "lofi", this.plugin.onSoundscapeChange()), this.plugin.settings.musicCollections.splice(index, 1), this.plugin.settings.myMusicIndex = [], this.plugin.settings.myMusicFolderPath = "", this.plugin.saveSettings(), this.display(), this.plugin.populateChangeSoundscapeButton();
              },
      ...
      component.setButtonText("Add Music Collection").setCta().onClick(() => {
        let id = v4_default();
...
var SoundscapesPlugin = class extends import_obsidian6.Plugin {
  ...
  async onload() {
    await this.loadSettings(), this.currentTrackIndex = this.settings.currentTrackIndex;
    let activeCollection = this.getActiveMusicCollection();
    activeCollection && activeCollection.currentTrackIndex !== void 0 && (this.currentTrackIndex = activeCollection.currentTrackIndex), this.settingsObservable = new Observable_default(this.settings), this.localPlayerStateObservable = new Observable_default({}), this.debouncedSaveSettings = (0, import_obsidian6.debounce)(this.saveSettings, 500, !0), this.versionCheck(), this.statusBarItem = this.addStatusBarItem(), this.statusBarItem.addClass("soundscapesroot"), this.createControls(), this.createPlayer(), this.onSoundscapeChange(this.settings.autoplay), this.registerView(
    ...
    ), this.ribbonButton.hide(), this.addSettingTab(new SoundscapesSettingsTab(this.app, this)), this.isMusicCollectionActive() && (this.ribbonButton.show(), setTimeout(() => {
  ...
  onunload() {
    this.reindexTimer && clearTimeout(this.reindexTimer), this.closeVolumePopupHandler && document.removeEventListener("click", this.closeVolumePopupHandler);
  }
  /**
  ...
  getActiveMusicCollection() {
    if (this.settings.soundscape.startsWith("MUSIC_COLLECTION_")) {
      let id = this.settings.soundscape.split("MUSIC_COLLECTION_")[1];
  ...
  async indexMusicLibrary() {
    ...
    let collection = this.getActiveMusicCollection();
    if (this.reindexTimer && clearTimeout(this.reindexTimer), !collection || collection.folderPath.trim() === "") {
    ...
    let musicPromises = getAllMusicFiles_default(collection.folderPath).map(async (filePath) => {
      let metadata = await import_music_metadata.default.parseFile(filePath, {
        skipCovers: !0
      });
      return {
        fileName: import_path2.default.basename(filePath),
        fullPath: filePath,
        title: metadata.common.title || import_path2.default.basename(filePath).replace(/\.[^.]+$/gi, ""),
        artist: metadata.common.artist,
        album: metadata.common.album,
        duration: metadata.format.duration
      };
    }), songs = await Promise.all(musicPromises);
    console.timeEnd("MusicIndex"), collection.musicIndex = songs, this.settings.myMusicIndex = songs, this.settings.myMusicFolderPath = collection.folderPath, this.currentTrackIndex >= songs.length && (this.currentTrackIndex = 0), new import_obsidian6.Notice(
  ...
  resetReindexTimer() {
    this.reindexTimer && clearTimeout(this.reindexTimer), this.settings.reindexFrequency !== "never" && (this.reindexTimer = setTimeout(
      () => this.indexMusicLibrary(),
      6e4 * parseInt(this.settings.reindexFrequency)
    ));
  }
    ...
    this.statusBarItem.createEl("script", {
      ...
      this.player = new YT.Player("player", {
        ...
        events: {
          ...
          onStateChange: (e) => {
            this.isMusicCollectionActive() || this.onStateChange.bind(this)(e);
          }
        }
      }), setInterval(() => {
        var _a, _b, _c;
        let ytPlayer = this.player;
        if (ytPlayer && typeof ytPlayer.getPlayerState == "function" && ytPlayer.getPlayerState() === 1 && !this.isMusicCollectionActive()) {
          let currentTime = ytPlayer.getCurrentTime() || 0, duration = ytPlayer.getDuration() || 0, currentState = this.localPlayerStateObservable.getValue(), author = "YouTube Creator";
          if (typeof ytPlayer.getVideoData == "function") {
            let videoData = ytPlayer.getVideoData();
            videoData && videoData.author && (author = videoData.author);
          }
          if (this.soundscapeType === "CUSTOM" /* CUSTOM */) {
            let customSoundscape = this.getCurrentCustomSoundscape(), track = customSoundscape == null ? void 0 : customSoundscape.tracks[this.currentTrackIndex];
            track && (!track.author || !track.duration) && (track.author = author, track.duration = duration, this.saveSettings());
          }
          this.updateLocalPlayerState({
            currentTime,
            ...
            currentTrack: currentState.currentTrack ? {
              ...
              title: this.soundscapeType === "CUSTOM" /* CUSTOM */ ? ((_b = (_a = this.getCurrentCustomSoundscape()) == null ? void 0 : _a.tracks[this.currentTrackIndex]) == null ? void 0 : _b.name) || "" : ((_c = Soundscapes_default[this.settings.soundscape]) == null ? void 0 : _c.name) || "",
              artist: author,
              duration
            }
          });
  ...
  createControls() {
    ...
    }), (0, import_obsidian6.setIcon)(this.nextButton, "skip-forward"), this.nextButton.onclick = () => this.next(), this.repeatToggleButton = this.statusBarItem.createEl("button", {
      cls: "soundscapesroot-repeattogglebutton"
    }), this.updateRepeatToggleIcon(), this.repeatToggleButton.onclick = () => this.toggleRepeat();
    ...
    (0, import_obsidian6.setIcon)(changeSoundscapeButton, "list-music"), this.changeSoundscapeSelect = changeSoundscapeButton.createEl(
    ...
    ), this.populateChangeSoundscapeButton(), this.changeSoundscapeSelect.addEventListener(
      "change",
      (event) => {
    ...
    }), this.toggleNowPlayingScroll(), this.volumeButton = this.statusBarItem.createEl("button", {
      cls: "soundscapesroot-volumebutton"
    }), this.volumeIconSpan = this.volumeButton.createSpan(), this.updateVolumeIcon(this.settings.volume), this.volumeButton.onclick = (e) => {
      e.stopPropagation(), this.volumePopup.hasClass("soundscapesroot-volumePopup--visible") ? this.volumePopup.removeClass("soundscapesroot-volumePopup--visible") : this.volumePopup.addClass("soundscapesroot-volumePopup--visible");
      ...
      this.volumePopup.hasClass("soundscapesroot-volumePopup--visible") && this.volumePopup.removeClass("soundscapesroot-volumePopup--visible");
    }, document.addEventListener("click", this.closeVolumePopupHandler), this.volumePopup.addEventListener("click", (e) => {
      e.stopPropagation();
    });
  }
  /**
  ...
  populateChangeSoundscapeButton() {
    if (!this.changeSoundscapeSelect)
      return;
    this.changeSoundscapeSelect.replaceChildren();
    let ambientGroup = this.changeSoundscapeSelect.createEl("optgroup", {
      attr: { label: "\u{1F4FB} Ambient Streams" }
    });
    Object.values(Soundscapes_default).forEach((soundscape) => {
    ...
    let validCustomPlaylists = this.settings.customSoundscapes.filter(
      (cs) => cs.tracks && cs.tracks.length > 0
    );
    if (validCustomPlaylists.length > 0) {
      let youtubeGroup = this.changeSoundscapeSelect.createEl("optgroup", {
        attr: { label: "\u25B6 \u{1D688}\u{1D698}\u{1D69E}\u{1D69D}\u{1D69E}\u{1D68B}\u{1D68E}" }
      });
    ...
    if (this.settings.musicCollections && this.settings.musicCollections.length > 0) {
      let localGroup = this.changeSoundscapeSelect.createEl("optgroup", {
        attr: { label: "\u{1F4C1} Collections" }
      });
  ...
  play() {
    var _a, _b;
    this.soundscapeType === "STANDARD" /* STANDARD */ && Soundscapes_default[this.settings.soundscape].isLiveVideo && ((_a = this.player) == null || _a.seekTo(this.player.getDuration())), this.isMusicCollectionActive() ? this.localPlayer.src === location.href && this.settings.myMusicIndex.length > 0 ? (this.currentTrackIndex = 0, this.onSoundscapeChange(!0)) : this.localPlayer.play() : (_b = this.player) == null || _b.playVideo();
  }
  /**
  ...
  pause() {
    var _a;
    this.isMusicCollectionActive() ? this.localPlayer.pause() : (_a = this.player) == null || _a.pauseVideo();
  }
  /**
  ...
  previous() {
    if (this.soundscapeType === "CUSTOM" /* CUSTOM */) {
      ...
      this.isMusicCollectionActive() && (this.settings.playMode === "shuffle" ? (this.shuffleQueue.length === 0 && (this.shuffleQueue = createShuffleQueue_default(
   ...
   */
  seek(time) {
    this.isMusicCollectionActive() && (this.localPlayer.currentTime = time);
  }
  /**
   ...
   */
  toggleShuffle() {
    this.settings.playMode = this.settings.playMode === "shuffle" ? "repeat" : "shuffle", this.saveSettings(), this.settings.playMode === "shuffle" && (this.shuffleQueue = [], this.shuffleQueueSpot = 0), this.updateRepeatToggleIcon();
  }
  toggleRepeat() {
    this.settings.playMode = this.settings.playMode === "shuffle" ? "repeat" : "shuffle", this.saveSettings(), this.settings.playMode === "shuffle" && (this.shuffleQueue = [], this.shuffleQueueSpot = 0), this.updateRepeatToggleIcon();
  }
  updateRepeatToggleIcon() {
    this.settings.playMode === "repeat" ? (0, import_obsidian6.setIcon)(this.repeatToggleButton, "repeat") : (0, import_obsidian6.setIcon)(this.repeatToggleButton, "shuffle"), this.repeatToggleButton.onclick = () => this.toggleRepeat();
  }
  /**
  ...
  changeSoundscape(soundscape) {
    var _a;
    let prevCollection = this.getActiveMusicCollection();
    if (prevCollection && (prevCollection.currentTrackIndex = this.currentTrackIndex), this.settings.soundscape = soundscape, this.settings.soundscape.startsWith("CUSTOM_") && (this.currentTrackIndex = 0), this.isMusicCollectionActive()) {
      let collection = this.getActiveMusicCollection();
      if (collection) {
        this.settings.myMusicIndex = collection.musicIndex, this.settings.myMusicFolderPath = collection.folderPath;
        let savedIndex = (_a = collection.currentTrackIndex) != null ? _a : 0;
        this.currentTrackIndex = savedIndex < this.settings.myMusicIndex.length ? savedIndex : 0;
      }
      this.shuffleQueue = [], this.shuffleQueueSpot = 0, collection && collection.musicIndex.length === 0 ? this.indexMusicLibrary() : this.resetReindexTimer(), this.ribbonButton.show();
    } else
      this.ribbonButton.hide();
    this.onSoundscapeChange(), this.saveSettings();
  }
  /******************************************************************************************************************/
  ...
  onStateChange({ data: state }) {
    let customSoundscape = this.getCurrentCustomSoundscape();
    this.soundscapeType === "STANDARD" /* STANDARD */ ? (this.previousButton.hide(), this.nextButton.hide(), this.repeatToggleButton.hide()) : (this.previousButton.show(), this.nextButton.show(), this.repeatToggleButton.show());
    let resolvedTrack;
    if (this.isMusicCollectionActive())
      resolvedTrack = this.settings.myMusicIndex[this.currentTrackIndex];
    else if (this.soundscapeType === "CUSTOM" /* CUSTOM */) {
      let track = customSoundscape == null ? void 0 : customSoundscape.tracks[this.currentTrackIndex], ytPlayer = this.player, author = (track == null ? void 0 : track.author) || (track == null ? void 0 : track.channelName) || "YouTube Creator", duration = (track == null ? void 0 : track.duration) || 0;
      if (ytPlayer && typeof ytPlayer.getVideoData == "function") {
        let videoData = ytPlayer.getVideoData();
        if (videoData && videoData.author && (author = videoData.author), typeof ytPlayer.getDuration == "function") {
          let ytDuration = ytPlayer.getDuration();
          ytDuration > 0 && (duration = ytDuration);
        }
      }
      track && (!track.author || !track.duration) && (track.author = author, track.duration = duration, this.saveSettings()), resolvedTrack = {
        fileName: (track == null ? void 0 : track.name) || "",
        fullPath: (track == null ? void 0 : track.id) || "",
        title: (track == null ? void 0 : track.name) || "",
        artist: author,
        album: (customSoundscape == null ? void 0 : customSoundscape.name) || "YouTube Playlist",
        duration
      };
    } else {
      let ambient = Soundscapes_default[this.settings.soundscape];
      resolvedTrack = {
        fileName: (ambient == null ? void 0 : ambient.name) || "",
        fullPath: (ambient == null ? void 0 : ambient.youtubeId) || "",
        title: (ambient == null ? void 0 : ambient.name) || "",
    ...
    switch (state) {
      ...
      case 2 /* PAUSED */:
        ...
        break;
      case 0 /* ENDED */:
        this.settings.playMode === "repeat" && this.isMusicCollectionActive() || (this.soundscapeType === "CUSTOM" /* CUSTOM */ || this.isMusicCollectionActive()) && this.next(), this.onSoundscapeChange();
    }
  }
  /**
  ...
  onVolumeChange(e) {
    ...
    this.volumeSlider.value = e.target.value, (_a = this.player) == null || _a.setVolume(volume), this.localPlayer.volume = volume / 100, this.updateVolumeIcon(volume), this.settings.volume = volume, this.settingsObservable.setValue(this.settings), this.debouncedSaveSettings();
  }
  updateVolumeIcon(volume) {
    volume === 0 ? (0, import_obsidian6.setIcon)(this.volumeIconSpan, "volume-x") : volume <= 50 ? (0, import_obsidian6.setIcon)(this.volumeIconSpan, "volume-1") : (0, import_obsidian6.setIcon)(this.volumeIconSpan, "volume-2");
  }
  /**
  ...
  onSoundscapeChange(autoplay = !0) {
    ...
    if (this.nowPlaying) {
      if (this.settings.soundscape.startsWith("CUSTOM_") ? this.soundscapeType = "CUSTOM" /* CUSTOM */ : this.isMusicCollectionActive() ? this.soundscapeType = "MY_MUSIC" /* MY_MUSIC */ : this.soundscapeType = "STANDARD" /* STANDARD */, this.soundscapeType === "CUSTOM" /* CUSTOM */) {
        let customSoundscape = this.getCurrentCustomSoundscape();
        ...
        if (track) {
          ...
          let ext = ((_c = track.fullPath.split(".").pop()) == null ? void 0 : _c.toLowerCase()) || "mp3";
          this.localPlayer.src = `data:${getMimeType(ext)};base64,${base64Data}`, this.nowPlaying.setText(`${track.title} - ${track.artist}`), autoplay ? this.localPlayer.play() : this.onStateChange({ data: 2 /* PAUSED */ });
        } else
          this.localPlayer.src = "", this.nowPlaying.setText(""), this.onStateChange({ data: 2 /* PAUSED */ });
        this.statusBarItem.addClass("soundscapesroot--hideyoutube"), (_d = this.player) == null || _d.pauseVideo();
      } else
        (_e = this.player) == null || _e.loadVideoById({
          videoId: Soundscapes_default[this.settings.soundscape].youtubeId
        }), Soundscapes_default[this.settings.soundscape].isLiveVideo && ((_f = this.player) == null || _f.seekTo(this.player.getDuration())), this.nowPlaying.setText(
          Soundscapes_default[this.settings.soundscape].nowPlayingText
        ), autoplay || (_g = this.player) == null || _g.pauseVideo(), this.statusBarItem.removeClass("soundscapesroot--hideyoutube"), this.localPlayer.pause();
  ...
  async loadSettings() {
    ...
    this.settings = Object.assign({}, DEFAULT_SETTINGS, data), data.myMusicFolderPath && data.myMusicFolderPath.trim() !== "" && this.settings.musicCollections.length === 0 && (this.settings.musicCollections.push({
      id: v4_default(),
      name: "My Music",
      folderPath: data.myMusicFolderPath,
      musicIndex: data.myMusicIndex || [],
      currentTrackIndex: 0
    }), this.settings.soundscape = `MUSIC_COLLECTION_${this.settings.musicCollections[0].id}`, this.saveSettings()), this.settings.soundscape === "MY_MUSIC" /* MY_MUSIC */ && (this.settings.soundscape = "lofi"), this.settings.soundscape.startsWith("MUSIC_COLLECTION_") && !this.getActiveMusicCollection() && (this.settings.soundscape = "lofi");
  }
  /**
  ...
  async saveSettings() {
    ...
    let collection = this.getActiveMusicCollection();
    collection && (collection.currentTrackIndex = this.currentTrackIndex), this.settingsObservable.setValue(this.settings), await this.saveData(this.settings);
  }
  /**
